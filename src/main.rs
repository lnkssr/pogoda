...
async fn fetch_ip_info(url: &str) -> Result<IpApiResponse, Box<dyn Error>> {
    ...
    let response = client.get(url).send().await?.json::<IpApiResponse>().await?;
    Ok(response)
}
...
fn print_weather_data(weather_response: &WeatherResponse) {
    ...
    if let Some(current) = &weather_response.current {
        println!("Текущая температура: {:.2}°C", current.temperature_2m.unwrap_or(0.0));
        println!("Скорость ветра: {:.2} м/с", current.wind_speed_10m.unwrap_or(0.0));
        println!("Вероятность осадков: {:.2}%", current.precipitation_probability.unwrap_or(0.0));
    } else {
        println!("Не удалось получить текущие данные о погоде.");
    }

    // Найти последний индекс для часовых данных
    let last_index = weather_response.hourly.temperature_2m.len().saturating_sub(1);
    ...
    let last_precipitation = weather_response.hourly.precipitation_probability.get(last_index).unwrap_or(&0.0);
